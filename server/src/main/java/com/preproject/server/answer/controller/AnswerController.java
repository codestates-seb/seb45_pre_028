...
public class AnswerController {
    ...
    // 답변이 등록된걸로 고유의 답변id가 부여됐을테니 질문번호를 찾지 않고 로직 구현
    // 답변이 한 페이지에 여러 개의 답변이 구현되려면 페이지네이션을 적용하면 될듯함!
    @GetMapping("/answer/{answer-id}")
    public ResponseEntity getAnswer(@PathVariable("answer-id") long answerId) {

        // 조회할 질문은 PathVariable의 경로를 통해 해당 값으로 조회 됨
    ...
    // 답변 삭제 (답변 id를 통해서 조회 후 삭제)
    @DeleteMapping("/answer/{answer-id}")
    public ResponseEntity deleteAnswer(@PathVariable("answer-id") long answerId) {

        // 삭제할 답변은 PathVarible의 경로를 통해 해당 값으로 조회 됨
